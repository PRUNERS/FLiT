...
INST_FLIT_CONFIG   := $(DESTDIR)$(PREFIX)/share/flit/scripts/flitconfig.py
...
install: $(TARGET)
	...
	@echo "script_dir = '$(abspath $(PREFIX))/share/flit/scripts'"              >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# flit documentation"                                                >> $(INST_FLIT_CONFIG)
	@echo "doc_dir = '$(abspath $(PREFIX))/share/flit/doc'"                     >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# compiled libflit.so"                                               >> $(INST_FLIT_CONFIG)
	@echo "lib_dir = '$(abspath $(PREFIX))/lib'"                                >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# flit C++ include files, primarily flit.h"                          >> $(INST_FLIT_CONFIG)
	@echo "include_dir = '$(abspath $(PREFIX))/include/flit'"                   >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# default configuration for flit init"                               >> $(INST_FLIT_CONFIG)
	@echo "config_dir = '$(abspath $(PREFIX))/share/flit/config'"               >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# current version"                                                   >> $(INST_FLIT_CONFIG)
	@echo "with open(os.path.join(config_dir, 'version.txt'), 'r') as version_file:" >> $(INST_FLIT_CONFIG)
	@echo "    version = version_file.read().strip()"                           >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# default data files such as Makefile.in and main.cpp"               >> $(INST_FLIT_CONFIG)
	@echo "data_dir = '$(abspath $(PREFIX))/share/flit/data'"                   >> $(INST_FLIT_CONFIG)
	@echo                                                                       >> $(INST_FLIT_CONFIG)
	@echo "# directory containing litmus tests"                                 >> $(INST_FLIT_CONFIG)
	@echo "litmus_test_dir = '$(abspath $(PREFIX))/share/flit/litmus-tests'"    >> $(INST_FLIT_CONFIG)
