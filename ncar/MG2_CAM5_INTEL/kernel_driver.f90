...
PROGRAM kernel_driver
    ...
    !INTEGER :: kgen_repeat_counter_mpi, kgen_repeat_counter_c
    ...
    WRITE (*,*) "["
#endif
    !I think the kgen_repeat_counter device wasn't working as intended -- I think the desired input set
    !was the cross product of kgen_mpi_rank_at and kgen_counter_at, so I revised to 2 nested loops
    ...
    DO kgen_mpi_rank = 1, 3
       DO kgen_counter = 1,3
          WRITE( kgen_mpi_rank_conv, * ) kgen_counter_at(kgen_mpi_rank)
          WRITE( kgen_counter_conv, * ) kgen_mpi_rank_at(kgen_counter)
          CALL GETCWD(kgen_filepath)
          kgen_filepath = trim(kgen_filepath) // "/micro_mg_tend2_0." // trim(adjustl(kgen_mpi_rank_conv)) // "." // trim(adjustl(kgen_counter_conv))
#else
